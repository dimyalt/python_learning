...
bot = telebot.TeleBot('1638491301:AAHqxXK9Iem9rC2fSz5mPGv_hWmJrrTnaFk') # Подключили токен телеграмм бота / We connected the bot's telegram token

@bot.message_handler(commands=['start']) # Обработка команды "start" / Processing the "start" command
def start_message(message):
    bot.send_message(message.chat.id, 'Привет, ты написал мне /start') # Ответ бота на команду "start" / Bot response to "start" command
...
def get_name(message):
    ...
    bot.register_next_step_handler(message, get_org_name);
    print(data)

def get_org_name(message):
    global get_org_name;
...
def get_type_game(message):
    ...
    get_name_user = message.from_user.id;
...
bot.polling(none_stop=True, interval=0) # Опрос телеграмма на входящие сообщения / Telegram survey for incoming messages
