...
book = load_workbook("C:\\Obrabotka\\Акцизы № 1.xlsx", read_only=True, data_only=True)  # Загружаем исходный файл
...
def obrabotka_tablici(book):
    ...
    for row in range(7, row_count):  # Начиная с 7й строки и до конца строк в табличной части
        ...
        reestr_row2.insert(23,
                           vichet_litri)  # Добавляем значение переменной в ячейку 23 столбца (Всего литры) рабочего файла
        kod_pokazatelya = sheet[row][
            33].value  # Переменная со значением ячейки из столбца 34 исходной таблицы (Коэффициент вычета)
        if kod_pokazatelya == 31 or kod_pokazatelya == 32:
            reestr_row2.insert(19,
                       30012)  # Добавляем значение переменной в ячейку 19 столбца (Код показ.) рабочего файла
        else:
            reestr_row2.insert(19,
                       30013)  # Добавляем значение переменной в ячейку 19 столбца (Код показ.) рабочего файла

        print(reestr_row2)  # Отображаем список
        sheet_r.append(reestr_row2)  # Добавляем список в строку рабочего файла

obrabotka_tablici(book)  # Запускаем функцию
book = load_workbook("C:\\Obrabotka\\Акцизы № 2.xlsx", read_only=True, data_only=True)  # Загружаем второй исходный файл

obrabotka_tablici(book)  # Запускаем функцию с данными второй таблицы
